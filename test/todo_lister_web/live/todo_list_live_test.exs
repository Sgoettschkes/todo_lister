defmodule TodoListerWeb.TodoListLiveTest do
  ...
  describe "Title editing" do
    ...
    test "syncs title updates between multiple clients via PubSub", %{
      conn: conn,
      todo_list: todo_list
    } do
      ...
      view1
      |> element("form[phx-submit='save_title']")
      |> render_submit(%{title: "PubSub Updated Title"})
    ...
    test "syncs todo item changes between multiple clients via PubSub", %{
      conn: conn,
      todo_list: todo_list
    } do
      # Start two LiveView clients for the same todo list
    ...
    test "preserves editing state during external updates (optimistic locking)", %{
      conn: conn,
      todo_list: todo_list
    } do
      # Start two LiveView clients for the same todo list
      ...
      |> render_submit(%{text: "Item being edited"})
    ...
    test "detects and warns about text conflicts during concurrent edits", %{
      conn: conn,
      todo_list: todo_list
    } do
      ...
      view2
      |> element("form[phx-submit='save_item']")
      |> render_submit(%{text: "Changed by client 2"})
      ...
      # Still in edit mode
      assert html1 =~ "input"
      # Preserves original version being edited
      assert html1 =~ "Original text"
    end
  end
  ...
  describe "Todo items functionality" do
    ...
    test "hard delete soft deletes item from UI but keeps it in database", %{
      conn: conn,
      todo_list: todo_list
    } do
  ...
  describe "Todo list soft delete" do
    test "delete_todo_list soft deletes list from queries but keeps it in database", %{
      todo_list: todo_list
    } do
